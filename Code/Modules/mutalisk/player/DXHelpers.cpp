...
int calcTextureSize(IDirect3DTexture9 const& texture)
{
	...
	for ( int i = 0; i < mipMapLevels; ++i ) {
		...
		bytes +=
			max( 1, (int)sd.Width / divisor ) *
			max( 1, (int)sd.Height / divisor ) * multiplier;
	}
	return bytes;
}


CResultCheck::CResultCheck( char const* errorMessage_, char const* messagePrefix_, char const* messagePostfix_ )
...
CResultCheck& CResultCheck ::operator= ( HRESULT result )
{
	if( FAILED(result) )
	{
