...
//		BoneMapT						boneMap;
		...
		void setClip(mutant::anim_character_set& animCharSet, int clipIndex, bool looping)
		{
	...
	void setClip(int clipIndex, bool looping = true)
	{
		ASSERT(mResources.animCharSet.get());
		mState.setClip(*mResources.animCharSet, clipIndex, looping);

		ASSERT(mState.hierarchy);
		mapNodesToHierarchy(*mState.hierarchy, mBlueprint.lights, mBlueprint.lightCount, mState.light2XformIndex);
		mapNodesToHierarchy(*mState.hierarchy, mBlueprint.cameras, mBlueprint.cameraCount, mState.camera2XformIndex);
		mapNodesToHierarchy(*mState.hierarchy, mBlueprint.actors, mBlueprint.actorCount, mState.actor2XformIndex);
		
		mState.bone2XformIndex.resize(mBlueprint.meshCount);
		for(size_t q = 0; q < mBlueprint.meshCount; ++q)
			if( mResources.meshes[q].blueprint->skinInfo )
				mapBoneToHierarchy(*mState.hierarchy,
					mResources.meshes[q].blueprint->skinInfo->bones, mResources.meshes[q].blueprint->skinInfo->boneCount,
