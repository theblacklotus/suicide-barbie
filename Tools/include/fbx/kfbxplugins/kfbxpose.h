...
/** This class contains the description of a Pose manager.
  ...
  *
  *     -# If this KFbxPose object stores "Bind Poses", then
  ...
  *
  *     -# If a node is already inserted in the KFbxPose internal list,
...
public:
		...
		/** Get the list of Poses objects that contains the node with name pNodeName.
		  ...
		  * \param pManager Is the manager owning the poses and scenes.
		  * \param pNode The node we are exploring.
		  * \param pPosesList List of Bind/RestPoses that have the node.
		  * \param pIndexList Indices of the node in the corresponding poses lists.
		  * \return \c true if the node belongs to at least one Pose (either a BindPose or a RestPose).
		  * \remarks The pPosesList and pIndexList are filled by this method.
		...
		/** Get the list of Poses objects that contains the node with name pNodeName.
		  * \param pScene Is the scene owning the poses.
		  * \param pNode The node we are exploring.
		  * \param pPosesList List of Bind/RestPoses that have the node.
		  * \param pIndexList Indices of the node in the corresponding poses lists.
		  * \return \c true if the node belongs to at least one Pose (either a BindPose or a RestPose).
		...
		/** Get the list of BindPose objects that contains the node with name pNodeName.
		  * This method will look in all the poses of all the scenes.
		  * \param pManager Is the manager owning the poses.
		  * \param pNode The node we are exploring.
		  * \param pPosesList List of BindPoses that have the node.
		  * \param pIndexList Indices of the node in the corresponding poses lists.
		...
		/** Get the list of RestPose objects that contains the node with name pNodeName.
		  ...
		  * \param pManager Is the manager owning the poses.
		  * \param pNode The node we are exploring.
		  * \param pPosesList List of RestPoses that have the node.
		  * \param pIndexList Indices of the node in the corresponding poses lists.
		...
		/** Return the type ID of this class.
		  * \return Return KFbxObject::ePOSE.
		  */
...
protected:
	static char const* GetNamePrefix() { return POSE_PREFIX; }
