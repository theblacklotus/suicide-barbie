	...
	*/
	...
	public:
		...
		*/
		...
		// Get the first key in the children specified by the curve mask (NULL = all children)
		// DEPRECATED: use GetFirstKeyTime or GetCurveWithFirstKey instead.
		// K_DEPRECATED void KeyGetFirst(KFCurveKey*& lKey, bool *pCurveMask = NULL, bool pRecursiveInLayers = false);
		// Get the last key in the children specified by the curve mask (NULL = all children)
		// DEPRECATED: use GetLastKeyTime or GetCurveWithLastKey instead.
		// K_DEPRECATED void KeyGetLast(KFCurveKey*& lKey, bool *pCurveMask = NULL, bool pRecursiveInLayers = false);
	...
	protected:
		struct NameInfo 
		{
			...
			KProperty*	mProperty;	// Helper to display enum values in fcurve editor
			int		mRefCount;
		}*mNameInfo;
