...
enum
{
	...
	*/
	...
	public:
		...
		* Call function KFCurve::KeyModifyEnd(int pKeyCount) after modification 
		* of the keys are completed.
		* \param pKeyCount Number of keys to pre-allocate in the function curve.
		* This parameter specifies the total number of keys in the function
		* curve key buffers.
		* 
		...
		*	\param pKeyCount Number of valid keys set in the function curve key 
		* buffers.
		* 
		...
		* \remarks Result is undetermined if function curve has no key or index 
		* is out of bounds.
		* \remarks This function is deprecated. Use KFCurve functionnality to access 
		* or modify key data.
		*/
		K_DEPRECATED KFCURVE_INLINE KFCurveKey& KeyGetRaw(kFCurveIndex pIndex);

		/** Get pointer to key at given index (DEPRECATED).
		* \remarks Result is undetermined if function curve has no key or index 
		* is out of bounds. The keys are not necessarily stored in a continuous 
		* buffer so incrementing or decrementing the returned pointer may not 
		* lead to another key.
		* \remarks This function is deprecated. Use KFCurve functionnality to access 
		* or modify key data.
		*/
		K_DEPRECATED KFCURVE_INLINE KFCurveKey* KeyGetPtr(kFCurveIndex pIndex); 	
	...
	void KeySetRightDerivativeInfo(kFCurveIndex pIndex, KFCurveTangeantInfo pValue, bool pForceDerivative = false);
		...
		*	\param pIndex Index of key. Key index range is between 0 and 
		* KFCurve::KeyGetCountAll() - 1.
		*	\return Reference to a key. 
		* \remarks Result is undetermined if function curve has no key or index 
		* is out of bounds.
		* \remarks This function is deprecated. Use KFCurve functionnality to access 
		* or modify key data.
		...
		*	\param pIndex Index of key. Key index range is between 0 and 
		* KFCurve::KeyGetCountAll() - 1.
		*	\return Time of key at given index. 
		* \remarks Result is undetermined if function curve has no key or index 
		* is out of bounds.
		*/
		...
		*	\param pIsIso Pointer to variable set to \c true by the function if 
		* all keys are separated by equal periods.
		*	\param pIsoPeriod Pointer to variable set to sampling period if all 
		* keys are separated by equal periods.
		*	\param pMaxPeriod Pointer to variable set to maximum period found.
		*	\return Average period between keys.
		* 
