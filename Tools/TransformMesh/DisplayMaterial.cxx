		...
		if (leMat)
		{
			...
			if (leMat->GetDirectArray().GetCount())
			{
				...
				for (int lCount = 0; lCount < leMat->GetDirectArray().GetCount(); lCount ++)
				{
					...
					DisplayColor("            Ambient: ", lMaterial->GetAmbient());
					DisplayColor("            Diffuse: ", lMaterial->GetDiffuse());
					DisplayColor("            Specular: ", lMaterial->GetSpecular());
					DisplayColor("            Emissive: ", lMaterial->GetEmissive());
					DisplayDouble("            Opacity: ", lMaterial->GetOpacity());
					DisplayDouble("            Shininess: ", lMaterial->GetShininess());
					DisplayDouble("            Reflectivity: ", lMaterial->GetReflectivity());	
					DisplayString("            Shading Model: ", lMaterial->GetShadingModel()); 
					DisplayString("");
				}
			}
		}
