#ifndef __MY_COM_PTR_H
#define __MY_COM_PTR_H
...
class ptr_adapt
{
	template<typename T> struct ValTSelector { };
	template<typename T> struct ValTSelector<T*> { typedef T type; };
	template<typename T> struct ValTSelector< com_ptr<T> > { typedef typename T::_PtrClass type; };

public:
	typedef typename ValTSelector<PtrT>::type value_type;

	ptr_adapt() {}
	ptr_adapt( PtrT const& p ) : ptr( p ) {}
